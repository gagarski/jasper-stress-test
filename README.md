...
normalize tolerant HTML fragments to XHTML before resolving CSS. A lightweight
Jsoup-only processor provides a comparison point without a general CSS engine.
...
17 baseline; Flying Saucer 10 requires Java 21. The Jsoup-only processor uses
the same existing Jsoup dependency.
...
| `--css4j-html-processor` | `false` | Selects the experimental CSS4J/Validator.nu processor. It cannot be combined with another processor-selection switch. |
| `--flying-saucer-html-processor` | `false` | Selects the experimental Flying Saucer/Jsoup processor. It cannot be combined with another processor-selection switch. |
| `--jsoup-html-processor` | `false` | Selects the Jsoup parser with a constrained ph-css cascade and Jasper-compatible styled-text semantics. It cannot be combined with another processor-selection switch. |
...
Run the Jsoup-only proof of concept concurrently:
...
This processor parses each fragment once with Jsoup and performs one recursive
DOM traversal. ph-css parses inline declarations and `<style>` elements, while
Jsoup matches selectors. The local resolver applies source order, selector
specificity, `!important`, inline precedence, inheritance, and CSS-wide
keywords. The ph-css shorthand registry expands `font` and `background`;
parsed expressions remain typed until the final Jasper styled-text mapping.

The resulting computed properties are deliberately limited to what Jasper
styled text can represent: font family, weight, posture and size; foreground
and background; underline and strike-through; superscript/subscript; and
white-space handling. Legacy HTML tags, hyperlinks, line and block breaks, and
ordered/unordered list metadata are handled by the same traversal.

This remains a text converter rather than a browser engine. It ignores external
stylesheets, non-`all` media rules, nested at-rules, custom properties, generated
content, and layout. The checked-in report validates against the same
Swing-generated golden reference without Jsoup mode requiring EDT
serialization.
